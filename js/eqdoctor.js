...
$('#modal-primary').on('click', '#signbutton', function() {
    const $card = $('#eq_list').find('.col-12[data-id]');
    const id = $card.data('id'); 
    //console.log(id);
...
$('#signmodal').on('click', '#opandsignbutton', function() {
    const $card = $('#eq_list').find('.col-12[data-id]');
    const id = $card.data('id'); 
    //console.log(id);
    ...
    if (id === undefined) {
        ...
        const doctorOpinion = $('#doctoropinioninput').val();
        const digitalSignChecked = $('#digitalsigncheckbox').prop('checked');

        if (doctorOpinion === null || doctorOpinion === "" || digitalSignChecked === false) {
            // console.log('asd');
            // return;
        }
        // console.log(digitalSignChecked);

        $.ajax({
            url: 'query/doctorsign.php',
            type: 'POST',
            data: { id: id, doctorOpinion: doctorOpinion, digitalSignChecked: digitalSignChecked },
            dataType: "json",
            success: function(response) {
                if (response.success) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // Regenerate PDF by calling the same logic you had for generating the PDF
                            $.ajax({
                                url: 'generate_pdf.php',
                                type: 'POST',
                                data: { id: id },
                                success: function(response) {
                                    if (response.success) {
                                        const pdfUrl = response.pdf_url;
                                        $('#pdfIframe').attr('src', pdfUrl); // Reload the iframe with new PDF
                                    } else {
                                        alert('Error regenerating PDF: ' + response.error);
                                    }
                                },
                                error: function() {
                                    alert('Error generating PDF.');
                    ...
                    alert('Error updating doctor opinion and signature.');
                }
            },
            error: function() {
