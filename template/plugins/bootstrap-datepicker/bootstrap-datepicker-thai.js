 ...
 */
  function date_thai_db(bdate) {
    const date = new Date(bdate.replace(/^(\d+)\/(\d+)\/(\d+)$/, '$3-$2-$1'));
    const [y, m, d] = [date.getFullYear() - 543, (date.getMonth() + 1).toString().padStart(2, '0'), date.getDate().toString().padStart(2, '0')];
    return `${y}-${m}-${d}`;
  }
